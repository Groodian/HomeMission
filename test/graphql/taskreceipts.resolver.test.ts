...
describe('Task receipts resolver with', () => {
  ...
  it('TaskReceipts query returns an empty array when there are no task receipts', async () => {
  ...
  it('TaskReceipts query returns task receipts', async () => {
    await database.insertUsers();
    await database.insertHomes();
    await database.insertTaskTypes(1);
    await database.insertTasks();
  ...
  it('CreateTaskReceipt mutation returns error if task already has receipt', async () => {
    await database.insertUsers();
    await database.insertHomes();
    await database.insertTaskTypes(1);
    await database.insertTasks();
  ...
  it('CreateTaskReceipt mutation returns created task receipt', async () => {
    await database.insertUsers();
    await database.insertHomes();
    await database.insertTaskTypes(1);
